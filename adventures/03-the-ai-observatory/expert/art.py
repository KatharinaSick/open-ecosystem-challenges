...
class Art:
    def __init__(self):
        ...
        self.retrieval_counter = self.meter.create_counter(
            ...
            description="Counts the number of retrieved documents by category",
            unit="1"
        )
    ...
    def retrieve_context(self, user_input: str):
        ...
        with tracer.start_as_current_span("retrieval") as span:
            ...
            # DB semantic conventions for the Qdrant vector store
            span.set_attribute("db.system.name", "qdrant")
            span.set_attribute("db.collection.name", COLLECTION_NAME)
            span.set_attribute("db.operation.name", "similarity_search")
            ...
            span.set_attribute("gen_ai.retrieval.query", user_input)
            span.set_attribute("gen_ai.retrieval.top_k", MAX_RETRIEVAL_RESULTS)
            span.set_attribute("gen_ai.retrieval.documents_count", len(results))
        ...
        with tracer.start_as_current_span("rag.context_assembly") as span:
            span.set_attribute("context.categories", ",".join(retrieved_categories))
